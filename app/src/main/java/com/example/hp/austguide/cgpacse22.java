...
public class cgpacse22 extends AppCompatActivity {

    Double one,ten, two, three, four, five, six, seven, eight, nine, finalsum, p, q;
    EditText a, b, c, d, e, f, g, h, i,j;
    public static String cse11marks;
    TextView ss;
    double finalresultcg;
    double onegrd,tengrd, twogrd, threegrd, frgrd, fvegrd, sixgrd, sevgrd, eightgrd, ninegrd;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_cgpacse22);
        a = (EditText) findViewById(R.id.hummarks1);
        b = (EditText) findViewById(R.id.chemmarks1);
        c = (EditText) findViewById(R.id.phymarks1);
        d = (EditText) findViewById(R.id.csemarks1);
        e = (EditText) findViewById(R.id.mathmarks1);
        f = (EditText) findViewById(R.id.humlabmarks1);
        g = (EditText) findViewById(R.id.phylabmarks1);

        h = (EditText) findViewById(R.id.clabmarks1);

        i = (EditText) findViewById(R.id.c1108labmarks1);
        j= (EditText) findViewById(R.id.numlabmarks1);


    }
    public void onclicksz (View view)
    {
        MediaPlayer gh= MediaPlayer.create(this,R.raw.beep);
        try {
            one = Double.parseDouble(a.getText().toString());
            two = Double.parseDouble(b.getText().toString());
            three = Double.parseDouble(c.getText().toString());
            four = Double.parseDouble(d.getText().toString());
            five = Double.parseDouble(e.getText().toString());
            six = Double.parseDouble(f.getText().toString());
            seven = Double.parseDouble(g.getText().toString());
            eight = Double.parseDouble(h.getText().toString());
            nine = Double.parseDouble(i.getText().toString());

            ten = Double.parseDouble(j.getText().toString());
...
////////////////////////
            ...
            if (ten >= 80 && ten<= 100)
                tengrd = 4.0;
            else if (ten >= 75 && ten< 80)
                tengrd = 3.75;
            else if (ten >= 70 && ten < 75)
                tengrd = 3.5;
            else if (ten >= 65 && ten < 70)
                tengrd = 3.25;
            else if (ten >= 60 && ten < 65)
                tengrd = 3.0;
            else if (ten >= 55 && ten < 60)
                tengrd = 2.75;
            else if (ten >= 50 && ten < 55)
                tengrd = 2.50;
            else if (ten >= 45 && ten < 50)
                tengrd = 2.25;
            else if (ten >= 40 && ten < 45)
                tengrd = 2.0;
            else if (ten >= 0 && ten< 40)
                tengrd = 0;
            /////////////////////////////////////////


            p = (onegrd) * (3.0) + (twogrd) * (3.0) + (threegrd) * (3.0) + (frgrd) * (3.0) + (fvegrd) * (3.0) + (sixgrd) * (1.5) + (sevgrd) * (1.5) + (eightgrd) * (0.75) + (ninegrd) * (0.75)+(tengrd) * (0.75);

            q = (3.0) * (5) + ((1.5) * (2)) +(( 0.75)*(3));
            finalresultcg = p / q;
            ...
            Intent i = new Intent(this, allcgpaview.class);
            i.putExtra(cse11marks, pass);

            startActivity(i);
        }catch (Exception e)
        {
            gh.start();
            Toast.makeText(getApplicationContext(), "any of your marks feild is empty!!! please check it :) ", Toast.LENGTH_LONG).show();
        }
